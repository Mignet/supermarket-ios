...
@implementation JFAnimatedGifFrame
@synthesize data, delay, disposalMethod, area, header;
@end

@implementation JFGifImageView
@synthesize GIF_frames;
@synthesize fSpeedRate = _fSpeedRate;
...
+ (NSMutableArray*)getGifFrames:(NSData*)gifImageData {
	JFGifImageView* gifImageView = [[JFGifImageView alloc] initWithGIFData:gifImageData];
	if (!gifImageView) {
		return nil;
	}
	
	return gifImageView.GIF_frames;
}
...
- (id)initWithGIFData:(NSData*)gifImageData {
	...
	if (![JFGifImageView isGifImage:gifImageData]) {
...
- (void)loadImageData {
	...
	int i = 0;
	JFAnimatedGifFrame *lastFrame = nil;
	for (UIImage *image in array)
	{
		// Get Frame
		JFAnimatedGifFrame *frame = [GIF_frames objectAtIndex:i];
	...
	double total = 0;
	for (JFAnimatedGifFrame *frame in GIF_frames) {
		total += frame.delay;
	}
	
	[self setFSpeedRate:_fSpeedRate];
}
...
- (void) GIFReadExtensions {
	...
	while (cur[0] != 0x00)
    {
		...
		if (cur[0] == 0x04 && prev[0] == 0xF9)
		{
			...
			JFAnimatedGifFrame *frame = [[JFAnimatedGifFrame alloc] init];
...
- (void) GIFReadDescriptor {
	...
	JFAnimatedGifFrame *frame = [GIF_frames lastObject];
	frame.area = rect;
...
- (bool) GIFGetBytes:(int)length {
    ...
    {
		GIF_buffer = [NSMutableData dataWithData:
                      [GIF_pointer subdataWithRange:NSMakeRange(dataPointer, length)]];
        dataPointer += length;
		return YES;
	}
    else
...
- (NSData*) getFrameAsDataAtIndex:(int)index {
	...
	{
		return ((JFAnimatedGifFrame *)[GIF_frames objectAtIndex:index]).data;
	}
	else
	{
		return nil;
...
- (void)setFSpeedRate:(CGFloat)fSpeedRate
{
    if (fSpeedRate <= 0) {
        fSpeedRate = 1.0f;
    }
    _fSpeedRate = fSpeedRate;
	...
	[self setAnimationDuration:(total/_fSpeedRate)/100]; 
