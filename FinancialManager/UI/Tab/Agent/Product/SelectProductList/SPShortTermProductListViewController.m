//
//  SPShortTermProductListViewController.m
//  FinancialManager
//
...
@interface SPShortTermProductListViewController ()<XNFinancialProductModuleObserver, XNCommonModuleObserver,UniversalInteractWebViewControllerDelegate>
...
@implementation SPShortTermProductListViewController
...
- (void)loadDatas
{
    ...
    {
        _nPageIndex = 1;
        [[XNCommonModule defaultModule] requestBannerWithAdvPlacement:@"product_day"];
    }
    
    [[XNFinancialProductModule defaultModule] fmRequestSelectProductListWithCateId:@"1" pageIndex:[NSString stringWithFormat:@"%ld", _nPageIndex] pageSize:@"10"];
}
...
- (void)XNCommonModuleBannerDidReceive:(XNCommonModule *)module
{
    ...
    [self.tableView.mj_header endRefreshing];
    self.bannerString = module.shortTermProductImageUrl;
...
- (void)XNFinancialProductModuleSelectProductListDidReceive:(XNFinancialProductModule *)module
{
    ...
    XNFMProductCategoryListMode *mode = module.shortTermSelectProductsListMode;
    if (mode == nil)
    {
        return;
    }
