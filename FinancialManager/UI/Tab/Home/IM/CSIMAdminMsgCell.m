...
@implementation CSIMAdminMsgCell
...
- (void)refreshMsgContent:(EMMessage *)content avatorImageString:(NSString *)avatorImageString
{
    @autoreleasepool {
        ...
        UIImage * image = [_LOGIC getImageFromLocalBox:[NSString stringWithFormat:@"%@_userPic.png",[[NSString decryptUseDES:[_LOGIC getValueForKey:XN_USER_MOBILE_TAG]] md5]]];
        if (image) {
            
            [self.headerImageView setImage:image];
        }else
        {
            [self.headerImageView setImage:[UIImage imageNamed:@"XN_CS_IM_Chat_AdminHeaderImg.png"]];
        }
        ...
        if (msgBody.type == EMMessageBodyTypeText)
        {
            ...
            //表情映射
            NSString *contentString = [ConvertToCommonEmoticonsHelper convertToSystemEmoticons:[textMsgBody text]];
        ...
        else if(msgBody.type == EMMessageBodyTypeImage)
        {
            ...
            NSURL *url = [NSURL fileURLWithPath:imageBody.thumbnailLocalPath];
            
            if (imageBody.thumbnailLocalPath == nil || imageBody.thumbnailLocalPath.length < 1)
            {
                url = [NSURL URLWithString:imageBody.remotePath];
            }
            else
            {
                url = [NSURL fileURLWithPath:imageBody.thumbnailLocalPath];
            }
        ...
        [self.msgView mas_remakeConstraints:^(MASConstraintMaker *make) {
            ...
            make.trailing.mas_equalTo(tmpView.mas_trailing).offset(- 50);
...
- (UIImageView *)timeBgImageView
{
    if (!_timeBgImageView) {
        ...
        [_timeBgImageView setImage:[UIImage imageNamed:@"XN_CS_IM_Time_bg"]];
...
- (UIImageView *)headerImageView
{
    if (!_headerImageView) {
        ...
        [_headerImageView setImage:[UIImage imageNamed:@"XN_CS_IM_Chat_AdminHeaderImg.png"]];
    }
...
- (UIImageView *)indicateImageView
{
    if (!_indicateImageView) {
        ...
        [_indicateImageView setImage:[UIImage imageNamed:@"XN_CS_IM_Chat_AdminAnswer_right_bg.png"]];
        [_indicateImageView.layer setMasksToBounds:YES];
    }
...
- (UIImageView *)msgImageView
{
    if (!_msgImageView) {
        ...
        [_msgImageView setImage:[UIImage imageNamed:@"XN_CS_IM_Chat_AdminAsk_bg.png"]];
...
- (UILabel *)msgLabel
{
    if (!_msgLabel) {
        ...
        [_msgLabel setTextColor:[UIColor blackColor]];
...
- (UIView *)msgView
{
    if (!_msgView) {
        ...
        [self.indicateImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(tmpView.mas_top).offset(10);
        ...
        [self.msgImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.trailing.mas_equalTo(tmpView.mas_trailing).offset(-2);
        ...
        [self.msgLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.trailing.mas_equalTo(tmpView.mas_trailing).offset(- 17);
            make.bottom.mas_equalTo(tmpView.mas_bottom).offset( - 12);
        }];
        
        [self.msgPictureImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.leading.mas_equalTo(tmpView.mas_leading).offset(0.5);
            make.top.mas_equalTo(tmpView.mas_top).offset(0.5);
            make.trailing.mas_equalTo(tmpView.mas_trailing).offset(-2.5);
